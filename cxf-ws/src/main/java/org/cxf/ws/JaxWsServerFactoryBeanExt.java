 ...
 */
public class JaxWsServerFactoryBeanExt extends JaxWsServerFactoryBean {
	...
	private void buildHandlerChain(Server server) {
		...
		chain.addAll(builder.buildHandlerChainFromClass(getServiceBeanClass(),
				server.getEndpoint().getEndpointInfo().getName(), server.getEndpoint().getService().getName(),
				this.getBindingId()));
		for (Handler<?> h : chain) {
			injectResources(h);
		}
	...
	/**
	 * inject resources into servant. The resources are injected according
	 * to @Resource annotations. See JSR 250 for more information.
	 */
	/**
	 * @param instance
	 */
	...
	/**
	 * @param blockPostConstruct
	 * 			@PostConstruct method will not be called if this property is set
	 *            to true - this may be necessary in cases when the @PostConstruct
	 *            method needs to be called at a later stage, for example, when a
	 *            higher level container does its own injection.
	 */
	@Override
	...
	/**
	 * No injection or PostConstruct will be called if this is set to true. If the
	 * container has already handled the injection, this should be set to true.
	 *
	 * @param b
	 */
	@Override
